...
Describe "Switch-SqlDatabases unit tests" -Tag "Unit" {
    ...
    Mock Get-AzureRmSqlDatabase {
        $mock = '{ "ResourceGroupName": "dwp-foo-bar-rg", "ServerName": "dwp-foo-bar-sql", "DatabaseName": "dwp-foo-bar-db", "Location": "northeurope" }'
    ...
    It "Should do a single check and a single rename if the existing database does not exist" {
        ...
        .\Switch-SqlDatabases -ResourceGroupName dwp-foo-bar-rg -SQLServerName dwp-foo-bar-sql -ExistingDatabaseName foobar-db -ReplacementDatabaseName foobar-new
